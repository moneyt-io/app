...
class TransactionDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<List<TransactionDetails>> getTransactionDetailsForEntry(
          int transactionId) =>
  ...
  Future<List<TransactionEntries>> getTransactionsByType(
          String documentTypeId) =>
      (select(transactionEntry)
            ..where((t) =>
                t.documentTypeId.equals(documentTypeId) & t.active.equals(true))
            ..orderBy([(t) => OrderingTerm.desc(t.date)]))
          .get();
  ...
  Future<List<TransactionEntries>> getTransactionsByContact(int contactId) =>
      (select(transactionEntry)
            ..where(
                (t) => t.contactId.equals(contactId) & t.active.equals(true))
            ..orderBy([(t) => OrderingTerm.desc(t.date)]))
          .get();
  ...
  Future<List<TransactionEntries>> getTransactionsByDateRange(
      ...
      (select(transactionEntry)
            ..where((t) =>
                t.date.isBiggerOrEqual(Constant(startDate)) & t.date.isSmallerThan(Constant(endDate)) &
  ...
  Future<List<TransactionEntries>> getTransactionsByCreditCardPayment(
    ...
    final query = select(transactionEntry).join([
      innerJoin(transactionDetail,
          transactionDetail.transactionId.equalsExp(transactionEntry.id))
    ]);
  ...
  Future<List<TransactionEntries>> getTransactionsByPaymentTypeAndId(
      String paymentTypeId, int paymentId) async {
    final query = select(transactionEntry).join([
      innerJoin(transactionDetail,
          transactionDetail.transactionId.equalsExp(transactionEntry.id))
    ]);
