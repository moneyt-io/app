...
class $ContactsTable extends Contacts
    with TableInfo<$ContactsTable, ContactTableData> {
  @override
  final GeneratedDatabase attachedDatabase;
  final String? _alias;
...
class ContactTableData extends DataClass
  ...
  ContactTableData copyWithCompanion(ContactsCompanion data) {
    return ContactTableData(
      id: data.id.present ? data.id.value : this.id,
      name: data.name.present ? data.name.value : this.name,
  ...
  @override
  int get hashCode =>
      Object.hash(id, name, email, phone, notes, createdAt, updatedAt);
  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
...
class ContactsCompanion extends UpdateCompanion<ContactTableData> {
  final Value<int> id;
  final Value<String> name;
...
abstract class _$AppDatabase extends GeneratedDatabase {
  ...
  List<DatabaseSchemaEntity> get allSchemaEntities =>
      [categories, accounts, contacts, transactions];
}
...
final class $$AccountsTableReferences
  ...
  static MultiTypedResultKey<$TransactionsTable, List<Transaction>>
      _transactionAccountTable(_$AppDatabase db) =>
          MultiTypedResultKey.fromTable(db.transactions,
              aliasName: $_aliasNameGenerator(
                  db.accounts.id, db.transactions.accountId));

  $$TransactionsTableProcessedTableManager get transactionAccount {
    final manager = $$TransactionsTableTableManager($_db, $_db.transactions)
        .filter((f) => f.accountId.id($_item.id));

    final cache = $_typedResult.readTableOrNull(_transactionAccountTable($_db));
    return ProcessedTableManager(
        manager.$state.copyWith(prefetchedData: cache));
  }
}
...
class $$AccountsTableTableManager extends RootTableManager<
    ...
    Account,
    PrefetchHooks Function({bool transactionAccount})> {
  $$AccountsTableTableManager(_$AppDatabase db, $AccountsTable table)
      : super(TableManagerState(
          ...
          withReferenceMapper: (p0) => p0
              ...
              .toList(),
          prefetchHooksCallback: ({transactionAccount = false}) {
            return PrefetchHooks(
              db: db,
              explicitlyWatchedTables: [
                if (transactionAccount) db.transactions
              ],
              addJoins: null,
              getPrefetchedDataCallback: (items) async {
                return [
...
final class $$ContactsTableReferences
    extends BaseReferences<_$AppDatabase, $ContactsTable, ContactTableData> {
...
class $$ContactsTableTableManager extends RootTableManager<
    ...
    (ContactTableData, $$ContactsTableReferences),
    ContactTableData,
    PrefetchHooks Function({bool transactionsRefs})> {
